...
for it in range(10):
    ...
    for env_size in [2,3,4,5]:
        ...
        prev_sta = [None for _ in range(num_players)]
        ...
        for ts in range(1000):
            ...
            if(rand < epsilon):
                ...
                action = np.argmax(q_table[current_player, state, :])
            ...
            regret = (np.max(rewards[current_player, state, :]) - rewards[current_player, state, action])
            cum_reward += rewards[current_player, state, action] 
            cum_regret += regret 
            ...
            if(prev_sta[current_player] is not None):
                # replace np.max(q) with most recent reward?
                q_table[current_player, prev_sta[current_player], prev_act[current_player]] = q_table[current_player, prev_sta[current_player], prev_act[current_player]] + (alpha * (prev_rew[current_player] + (gamma * np.max(q_table[current_player, state, :]) - q_table[current_player, prev_sta[current_player], prev_act[current_player]])))

            prev_sta[current_player] = state
...
p.set_title("Cumulative Regret by Environment Size (Full Info)", fontsize = 20)
