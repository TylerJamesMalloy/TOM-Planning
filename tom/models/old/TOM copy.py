...
class TOM():
    ...
    def __init__(self, env, args):
        ...
        self.inference_buffer = TOMReplayBuffer(    buffer_size=int(1e6), 
                                                    observation_shape=self.observation_size, 
                                                    action_shape=(self.action_size,),
                                                    belief_shape=(self.args.belief_in,),
                                                    num_agents=len(self.agents))
        ...
        self.inference_net = Policy(input_size=self.args.state_size, output_size=self.action_size, layers=args.layers).to(self.args.device)
    ...
    def on_step(self):
        ...
        dones_pred = (dones_pred > rand_dones).float()

        rand_mask =  th.rand(next_masks_pred.shape).to(device=self.device)
        next_masks_pred = (next_masks_pred > rand_mask).float()
